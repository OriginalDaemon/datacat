...
def main():
    ...
    for i in range(3):
        ...
        session.update_state({
            "windows": {
                "open": current_windows,
                "active": window_name
            },
            "resources": {
                "memory_mb": 50.0 + (i + 1) * 25.0
            }
        })
    ...
    for cycle in range(5):
        ...
        session.log_metric("cpu_percent", cpu_usage, tags=["cycle:{}".format(cycle + 1)])
        session.log_metric("memory_mb", memory_usage, tags=["cycle:{}".format(cycle + 1)])
    ...
    session.log_event("settings_changed", {
        "changed": ["theme", "refresh_rate"]
    })
    session.heartbeat()
    ...
    print("\nYou can now:")
    print("1. Query the Grafana endpoint: curl http://localhost:8080/api/grafana/sessions")
    print("2. Get this session: curl http://localhost:8080/api/sessions/{}".format(session.session_id))
